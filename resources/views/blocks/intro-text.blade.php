...
--}}
@php($stats = [
    ['number' => 12, 'label' => 'Virtues','icon' => 'icons.apple'], 
    ['number' => 192, 'label' => 'Stories','icon' => 'icons.book-open'], 
    ['number' => 96, 'label' => 'Activities','icon' => 'icons.party-popper']])
        ...
        <ul class="flex flex-col md:flex-row  justify-center  max-w-5xl mt-8 md:mt-16 mx-auto">
            @foreach ($stats as $stat)
                ...
                <li class="w-full md:w-1/2 lg:w-1/3 px-2 mb-8">
                    <div class="border rounded-[20px] flex flex-col md:flex-row text-center md:text-left items-center md:items-start p-8 lg:p-4 shadow-lg">
                        {{-- <div>I</div> --}}
                        <div class="md:mr-2 mb-4">
                            <div class="bg-blue-50 flex items-center justify-center text-blue-400 rounded-lg p-2 w-10 h-10">
                                ...
                                @foreach ($ints as $int)
                                    <span class="vertical-number-counter" data-to="{!! $int !!}">
                                        <ul class="transition duration-1000 translate-y-0">
                                            @for ($i = 0; $i < 10; $i++)
                                                <li class=" {!! $i == $int ? 'set-width' : 'make-narrow' !!} transition duration-100 text-right">{!! $i !!}</li>
                                            @endfor
                                        </ul>
                                    </span>
                                @endforeach
